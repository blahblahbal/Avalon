...
namespace Avalon.Particles
{
    public class HellEmbers : Particle
    {
		...
		public HellEmbers(Vector2 velocity)
		{
			Velocity = velocity;
		}
        ...
        {
			if(texture == null)
				texture = ModContent.Request<Texture2D>("Avalon/Assets/Textures/HellEmbers");
            ...
            byte Sub = (byte)ai3;
            spriteBatch.Draw(texture.Value, DrawPos, frame, new Color(255,255 - Sub, 255 - Sub, 128) * Opacity, rotation, frameOrigin, 0.1f + (Opacity * ai1), SpriteEffects.None, 0);
            Sub = (byte)(ai3 * 0.295);
            for (int i = 0; i < 8; i++)
            {

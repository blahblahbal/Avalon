...
public class Sweetstem : ModTile
{
    ...
    public override IEnumerable<Item> GetItemDrops(int i, int j)
    {
        ...
        if (stage == PlantStage.Blooming)
        {
            ...
            yield return new Item(ModContent.ItemType<Items.Material.Herbs.Sweetstem>(), dropItemStack);
        }
    }
    ...
    public override void KillTile(int i, int j, ref bool fail, ref bool effectOnly, ref bool noItem)
    {
        ...
        {
            Item.NewItem(WorldGen.GetItemSource_FromTileBreak(i, j), new Vector2(i, j).ToWorldCoordinates(), ModContent.ItemType<SweetstemSeeds>(), secondaryItemStack);
        }
        if (stage == PlantStage.Blooming)
        {
            if (flag)

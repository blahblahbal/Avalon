...
public class ZirconSapling : ModTile
{
    ...
    public static bool AttemptToGrowZirconFromSapling(int x, int y, bool underground)
    {
        ...
        bool flag = GrowZirconTreeWithSettings(x, y, GemTree_Zircon);
    ...
    public static bool GrowZirconTreeWithSettings(int checkedX, int checkedY, GrowTreeSettings settings)
    {
        int i;
    ...
    public static void GrowZirconTreeFXCheck(int x, int y)
    {
        ...
        if (treeHeight > 0)
        {
            if (Main.netMode == 2)
            {
                NetMessage.SendData(112, -1, -1, null, 1, x, y, treeHeight, ModContent.GoreType<ZirconGemLeaves>());
            }
            if (Main.netMode == 0)
            {

...
public class ZirconTree : ModTile
{
    private readonly string TexturePath = "Avalon/Tiles/GemTrees/ZirconGemTree";
    ...
    {
        int dustType = ((WorldGen.genRand.Next(10) != 0) ? 1 : ModContent.DustType<ZirconDust>());
    ...
    private static void ShakeTree(int i, int j)
    {
        ...
        if (Main.netMode == 2)
        {
            NetMessage.SendData(112, -1, -1, null, 1, x, y, 1f, ModContent.GoreType<ZirconGemLeaves>());
        }
        if (Main.netMode == 0)
        {
            TreeGrowFX(x, y, 1, ModContent.GoreType<ZirconGemLeaves>(), hitTree: true);
        }
    }
    ...
    private void DrawTrees(int k, int l)
    {
        ...
        if (tile == null || !tile.HasTile || tile.TileType != ModContent.TileType<ZirconTree>())
        {
            return;
        }

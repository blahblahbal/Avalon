...
public class TourmalineTree : ModTile
{
    ...
    {
        int dustType = ((WorldGen.genRand.Next(10) != 0) ? 1 : ModContent.DustType<TourmalineDust>());
    ...
    public override IEnumerable<Item> GetItemDrops(int i, int j)
    {
        ...
        int dropItemStack = 1;
        SetGemTreeDrops(ModContent.ItemType<Tourmaline>(), ModContent.ItemType<TourmalineGemcorn>(), tileCache, ref dropItem, ref secondaryItem);
        if (dropItem == 3)
        {
    ...
    private static void ShakeTree(int i, int j)
    {
        ...
        if (Main.netMode == 2)
        {
            NetMessage.SendData(112, -1, -1, null, 1, x, y, 1f, ModContent.GoreType<TourmalineGemLeaves>());
        }
        if (Main.netMode == 0)
        {
            TreeGrowFX(x, y, 1, ModContent.GoreType<TourmalineGemLeaves>(), hitTree: true);
        }
    }

    private static void EmitTourmalineLeaves(int tilePosX, int tilePosY, int grassPosX, int grassPosY)
    {
        ...
        {
            Gore.NewGoreDirect(new EntitySource_Misc(""), vector, Utils.RandomVector2(Main.rand, -num3, num3), ModContent.GoreType<TourmalineGemLeaves>(), 0.7f + Main.rand.NextFloat() * 0.6f).Frame.CurrentColumn = Main.tile[tilePosX, tilePosY].TileColor;
        }
    }
    ...
    private void DrawTrees(int k, int l)
    {
        ...
        if (tile == null || !tile.HasTile || tile.TileType != ModContent.TileType<TourmalineTree>())
        {
            return;
        }

...
public class PeridotSapling : ModTile
{
    ...
    public static bool AttemptToGrowPeridotFromSapling(int x, int y, bool underground)
    {
        ...
        bool flag = GrowPeridotTreeWithSettings(x, y, GemTree_Peridot);
    ...
    public static bool GrowPeridotTreeWithSettings(int checkedX, int checkedY, GrowTreeSettings settings)
    {
        int i;
    ...
    public static void GrowPeridotTreeFXCheck(int x, int y)
    {
        ...
        if (treeHeight > 0)
        {
            if (Main.netMode == 2)
            {
                NetMessage.SendData(112, -1, -1, null, 1, x, y, treeHeight, ModContent.GoreType<PeridotGemLeaves>());
            }
            if (Main.netMode == 0)
            {

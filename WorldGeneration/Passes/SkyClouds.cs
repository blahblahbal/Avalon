...
namespace Avalon.WorldGeneration.Passes
{
	...
	internal class SkyClouds : GenPass
	{
		...
		{
			ushort tileType = (x > DistanceFromSideOfWorldWhereRaincloudsEnd && x < Main.maxTilesX - DistanceFromSideOfWorldWhereRaincloudsEnd) ? TileID.Cloud : TileID.RainCloud;
			...
			for (float height = 0; height < maxWidth; height += WorldGen.genRand.NextFloat(0.5f, 1f))
			{
				...
				for (float width = -startingWidth; width < startingWidth; width += 0.5f)
				{
					...
					{
						Utils.MakeCircle(x + (int)(width * height) + WorldGen.genRand.Next(-7, 7), y - (int)(height * 1) + WorldGen.genRand.Next(-13, -10), WorldGen.genRand.Next(6, 9) - 3, TileID.Dirt);
					}
					else Utils.MakeCircle(x + (int)(width * height) + WorldGen.genRand.Next(-7, 7), y - (int)(height * 2) + WorldGen.genRand.Next(-4, 4), WorldGen.genRand.Next(4, 7), tileType);
				}
			}
		...
		public static void GrowCloudIsland(int x, int y, int width, int height)
		{
			...
			{
				Utils.MakeCircle(i, y + height / 2, height / 2, TileID.Dirt);
			}

...
namespace Avalon.ModSupport.MLL.Items
{
	...
	public class BottomlessBloodBucket : ModItem
	{
		public override void SetStaticDefaults()
		{
			ItemID.Sets.AlsoABuildingItem[Type] = true;
			ItemID.Sets.DuplicationMenuToolsFilter[Type] = true;
			//ItemID.Sets.ShimmerTransformToItem[Type] = ModContent.ItemType<BloodAbsorbantSponge>();
		...
		public override void HoldItem(Player player)
		{
			if (!player.JustDroppedAnItem)
			{
				...
				{
					if (tile.LiquidType != LiquidLoader.LiquidType<Blood>())
					{
						return;
					}
				}
				SoundEngine.PlaySound(SoundID.SplashWeak, player.position);
				tile.LiquidType = LiquidLoader.LiquidType<Blood>();

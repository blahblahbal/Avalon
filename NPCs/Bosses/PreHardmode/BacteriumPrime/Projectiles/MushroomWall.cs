...
public class MushroomWall : ModProjectile
{
	...
	public override bool PreDraw(ref Color lightColor)
	{
		...
		Rectangle frameTop = new Rectangle(0, 0, TextureAssets.Projectile[Type].Value.Width, textureTop.Value.Height);
		for (int i = 0; i < (int)Math.Max(Math.Floor(Projectile.height / 20f), 1); i++)
		{
			...
			Main.EntitySpriteDraw(TextureAssets.Projectile[Type].Value, drawPos, frame, Lighting.GetColor((drawPos + Main.screenPosition).ToTileCoordinates()), 0f, new Vector2(TextureAssets.Projectile[Type].Value.Width / 2, TextureAssets.Projectile[Type].Value.Height), 1f, SpriteEffects.None, 0);
		}

		Vector2 drawPosBottom = Projectile.Bottom - Main.screenPosition;
		Main.EntitySpriteDraw(textureBottom.Value, drawPosBottom, frame, Lighting.GetColor((drawPosBottom + Main.screenPosition).ToTileCoordinates()), 0f, new Vector2(TextureAssets.Projectile[Type].Value.Width / 2, TextureAssets.Projectile[Type].Value.Height), 1f, SpriteEffects.None, 0);

		Vector2 drawPosTop = Projectile.Top - Main.screenPosition + new Vector2(0, -16);
		Main.EntitySpriteDraw(textureTop.Value, drawPosTop, frameTop, Lighting.GetColor((drawPosTop + Main.screenPosition).ToTileCoordinates()), 0f, new Vector2(TextureAssets.Projectile[Type].Value.Width / 2, 0), 1f, SpriteEffects.None, 0);
		return false;
	}

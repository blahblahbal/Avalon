	...
	public class PhantoplasmaBall : ModNPC
	{
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
		{
			...
			for (int i = 0; i < NPC.oldPos.Length; i++)
			{
				...
				Main.EntitySpriteDraw(texture.Value, drawPosOld, sourceRectangle, new Color(255, 125, 125, 0) * (1 - (i * (1/12f))) * 0.2f * NPC.Opacity, NPC.rotation, frameOrigin, NPC.scale, SpriteEffects.None, 0);
			}
			Main.EntitySpriteDraw(texture.Value, drawPos, sourceRectangle, new Color(255, 255, 255, 0) * 0.3f * NPC.Opacity, NPC.rotation, frameOrigin, NPC.scale * 1.1f, SpriteEffects.None, 0);
			Main.EntitySpriteDraw(texture.Value, drawPos, sourceRectangle, new Color(255, 255, 255, 0) * 0.15f * NPC.Opacity, NPC.rotation, frameOrigin, NPC.scale * 1.2f, SpriteEffects.None, 0);
			Main.EntitySpriteDraw(texture.Value, drawPos, sourceRectangle, new Color(255, 255, 255, 0) * NPC.Opacity, NPC.rotation, frameOrigin, new Vector2(NPC.scale, NPC.scale), SpriteEffects.None, 0);
			return false;
		}

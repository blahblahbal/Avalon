...
public class WallofSteelMouthEye : ModNPC
{
	...
	private void FireballPhase()
	{
		...
		if (Phase1FireballCounter > 90)
		{
			...
			while (f <= .1f)
			{
				fire = Projectile.NewProjectile(NPC.GetSource_FromAI(), fireballPos.X, fireballPos.Y, (float)((Math.Cos(rotation + f) * 12f) * -1), (float)((Math.Sin(rotation + f) * 12f) * -1), ModContent.ProjectileType<WoSCursedFireball>(), dmg, 6f);
		...
		if (NPC.ai[2] == 90)
		{
			...
			fire = Projectile.NewProjectile(NPC.GetSource_FromAI(), fireballPos.X, fireballPos.Y, (float)((Math.Cos(rotation + f) * 12f) * -1), (float)((Math.Sin(rotation + f) * 12f) * -1), ModContent.ProjectileType<WoSCursedFireball>(), Main.expertMode ? 70 : 55, 6f);
			Main.projectile[fire].timeLeft = 600;
			if (Main.netMode != NetmodeID.SinglePlayer)
			{
				NetMessage.SendData(MessageID.SyncProjectile, -1, -1, NetworkText.Empty, fire);

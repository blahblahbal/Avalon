...
public class WallofSteelLaserEye : ModNPC
{
		...
		if (NPC.ai[2] == 10)
		{
			...
			if (Phase1FireballCounter == 0)
			{
				if (NPC.ai[3] <= 0.3f)
				{
					...
					fire = Projectile.NewProjectile(NPC.GetSource_FromAI(), laserPos.X + vel.X * 40f, laserPos.Y + vel.Y * 40f, vel.X * 8f, vel.Y * 8f, ModContent.ProjectileType<WoSLaserSmall>(), Main.expertMode ? 70 : 55, 6f);
	...
	private void SingleLaserSpamPhase()
	{
		...
		if (NPC.ai[2] == 75)
		{
			int laser = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center.X + NPC.DirectionTo(NPC.PlayerTarget().Center).X * 40f, NPC.Center.Y + NPC.DirectionTo(NPC.PlayerTarget().Center).Y * 40f, NPC.velocity.X, NPC.velocity.Y, ModContent.ProjectileType<WoSLaserSmall>(), Main.expertMode ? 70 : 55, 4f);

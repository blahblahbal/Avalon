...
public class HallowSpit : ModNPC
{
    ...
    public override void AI()
    {
        if (NPC.target == 255)
        {
            ...
            float num157 = 6f;
            num157 = 7f;
            Vector2 vector15 = new Vector2(NPC.position.X + NPC.width * 0.5f, NPC.position.Y + NPC.height * 0.5f);
            float num158 = Main.player[NPC.target].position.X + Main.player[NPC.target].width / 2 - vector15.X;
            float num159 = Main.player[NPC.target].position.Y + Main.player[NPC.target].height / 2 - vector15.Y;

...
public class Leaping : ModBuff
{
    ...
    {
		if (player.IsOnGroundPrecise()) player.GetModPlayer<AvalonPlayer>().IcarusTimer = 0;
		player.GetModPlayer<AvalonPlayer>().IcarusTimer++;
		if (player.GetModPlayer<AvalonPlayer>().IcarusTimer % 10 == 0 && player.wingTime != 0)
			player.wingTime++;

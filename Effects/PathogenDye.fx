...
float4 ArmorBasic(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    float4 color2 = tex2D(uImage0, coords + float2((cloud * 2 - 3) / uImageSize0.x, (cloud - 1) / uImageSize0.y));
    float4 color = tex2D(uImage0, coords + float2((cloud * 0.2) / uImageSize0.x, (cloud * 0.2) / uImageSize0.y));

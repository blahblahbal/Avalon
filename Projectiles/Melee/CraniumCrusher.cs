...
public class CraniumCrusher : ModProjectile
{
	...
	public override bool PreDraw(ref Color lightColor)
	{
		...
		Vector2 offset = new Vector2((float)(TextureAssets.Projectile[Type].Width() * 1.2f * 0.25f), -(float)(TextureAssets.Projectile[Type].Height() * 1.2f * 0.25f));

		for (int i = 0; i < Projectile.oldPos.Length; i++)
		{
			Vector2 drawPosOld = Projectile.oldPos[i] - Main.screenPosition + Projectile.Size / 2f;
			Main.EntitySpriteDraw(after.Value, drawPosOld, frame, lightColor * (1 - (i * 0.2f)) * 0.25f, Projectile.oldRot[i], frame.Size() / 2f + offset, Projectile.scale, SpriteEffects.None, 0);
		}
		Main.EntitySpriteDraw(TextureAssets.Projectile[Type].Value, drawPos, frame, lightColor, Projectile.rotation, frame.Size() / 2f + offset, Projectile.scale, SpriteEffects.None, 0);

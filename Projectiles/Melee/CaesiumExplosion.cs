...
public class CaesiumExplosion : ModProjectile
{
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            Main.EntitySpriteDraw(texture.Value, DrawPos + new Vector2(0, Projectile.ai[0] * 0.05f).RotatedBy(i * MathHelper.PiOver4), frame, Color.Gray * 0.3f, Projectile.rotation, frameOrigin, Projectile.scale, SpriteEffects.None, 0);
        }
            Main.EntitySpriteDraw(texture.Value, DrawPos, frame, new Color(255,255,255,200), Projectile.rotation, frameOrigin, Projectile.scale, SpriteEffects.None, 0);

...
public class Mercury : ModProjectile
{
    ...
    public override void SendExtraAI(BinaryWriter writer)
    {
        ...
        positionNode ??= modPlayer.HandleMercury();
    ...
    public override void AI()
    {
        ...
        if (Projectile.ai[2] == 0)
        {
            ...
            {
                positionNode ??= modPlayer.ObtainExistingMercury(hostPosition);
            }
            const int radius = 30;
            const float speed = 1.2f;
            Vector2 target = Main.projectile[(int)Projectile.ai[1]].Center +
                             (Vector2.One.RotatedBy(
                                 (MathHelper.TwoPi / modPlayer.Mercury.Count * positionNode.Value) +
                                 modPlayer.MercuryRotation) * radius);

...
public class Jupiter : ModProjectile
{
    ...
    public override void SendExtraAI(BinaryWriter writer)
    {
        ...
        positionNode ??= modPlayer.HandleJupiter();
    ...
    public override void AI()
    {
        ...
        if (Projectile.ai[2] == 0)
        {
            ...
            {
                positionNode ??= modPlayer.ObtainExistingJupiter(hostPosition);
            }
            ...
            Vector2 target = Main.projectile[(int)Projectile.ai[1]].Center +
                             (Vector2.One.RotatedBy(
                                 (MathHelper.TwoPi / modPlayer.Jupiter.Count * positionNode.Value) +
                                 modPlayer.JupiterRotation) * radius);

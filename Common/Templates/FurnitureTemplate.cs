...
namespace Avalon.Common.Templates
{
    ...
    public abstract class ChandelierTemplate : FurnitureTemplate
    {
            ...
            for (int i = topLeftX; i < topLeftX + sizeX; i++)
            {
                for (int j = topLeftY; j < topLeftY + sizeY; j++)
                {
                    ...
                    if (tileDrawTexture != null)
                    {
                        ...
                        Vector2 vector6 = vector + new Vector2(0f + PositioningFix, vector4.Y + PositioningFix);
        ...
        public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            //}
            spriteBatch.End();
            Matrix m = Main.Transform;
            if (Main.GameViewMatrix.TransformationMatrix.M11 > 1)
            {
                m = Main.GameViewMatrix.EffectMatrix;
            }
            spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, null, Main.Rasterizer, null, m);
            if (Main.tile[i, j].TileFrameX % 54 == 0 && Main.tile[i, j].TileFrameY % 54 == 0)
            {
                DrawMultiTileVines(i, j, spriteBatch);
            }
            spriteBatch.End();
            spriteBatch.Begin();
            return false;

...
public class AvalonWorld : ModSystem
{
    ...
    public static void Convert(int i, int j, int convertType, int size = 4)
    {
            ...
            for (int l = j - size; l <= j + size; l++)
            {
                if (WorldGen.InWorld(k, l, 1) && Math.Abs(k - i) + Math.Abs(l - j) < Math.Sqrt(size * size + size * size))
                {
                    ...
                    if (convertType == 0)
                    {
                        ...
                        else if (Data.Sets.Tile.Conversion.ShortGrass[type])
                        {
                            Main.tile[k, l].TileType = (ushort)ModContent.TileType<ContagionShortGrass>();
                            WorldGen.SquareTileFrame(k, l, true);
                            NetMessage.SendTileSquare(-1, k, l, 1);
                        }
                        if (type == TileID.Plants2 || type == TileID.HallowedPlants2)
                        {
                            Main.tile[k, l].TileType = (ushort)ModContent.TileType<ContagionShortGrass>();
                            WorldGen.SquareTileFrame(k, l, true);
                            NetMessage.SendTileSquare(-1, k, l, 1);

...
internal class AvalonSpecialSeeds : ModHook
{
	...
	private void On_UIWorldCreation_ProcessSpecialWorldSeeds(On_UIWorldCreation.orig_ProcessSpecialWorldSeeds orig, string processedSeed)
	{
		...
		AvalonWorld.OopsAllCavesGen = false;
		if (processedSeed.ToLower() == "oopsallcaves" || processedSeed.ToLower() == "oops all caves" || processedSeed.ToLower() == "oops! all caves")
		{
			AvalonWorld.OopsAllCavesGen = true;
		}
	}
}
public class AvalonSpecialSeedsGenSystem : ModSystem
{
	...
	public override void ModifyWorldGenTasks(List<GenPass> tasks, ref double totalWeight)
	{
		...
		if (AvalonWorld.OopsAllCavesGen)
		{
			int index = tasks.FindIndex(genpass => genpass.Name.Equals("Dunes"));
			if (index != -1)
			{
				tasks.Insert(index + 1, new PassLegacy("Oops All Caves", new WorldGenLegacyMethod(delegate (GenerationProgress progress, GameConfiguration config)
				{
			...
			index = tasks.FindIndex(genpass => genpass.Name.Equals("Spawn Point"));
			if (index != -1)
			{
				tasks.Insert(index + 1, new PassLegacy("Oops All Caves Spawn Point", new WorldGenLegacyMethod(delegate (GenerationProgress progress, GameConfiguration config)
				{
					Main.spawnTileY += 300;
					GenerateSpawnArea(Main.spawnTileX - 5, Main.spawnTileY - 6);
				})));
			}
			...
			if (index != -1)
			{
				tasks.Insert(index + 1, new PassLegacy("Oops All Caves Lakes Fix", new WorldGenLegacyMethod(delegate (GenerationProgress progress, GameConfiguration config)
				{
					GenVars.worldSurfaceLow = 150;
				})));
			}
			index = tasks.FindIndex(genpass => genpass.Name.Equals("Lakes"));
			if (index != -1)
			{
				tasks.Insert(index + 1, new PassLegacy("Oops All Caves Lakes Fix Part 2", new WorldGenLegacyMethod(delegate (GenerationProgress progress, GameConfiguration config)
				{
					GenVars.worldSurfaceLow = 30;
				})));
			}
			...
			// TODO: make loot found elsewhere
			...
			index = tasks.FindIndex(genpass => genpass.Name.Equals("Planting Trees"));
			if (index != -1)
			{
				tasks.RemoveAt(index);
			}
			index = tasks.FindIndex(genpass => genpass.Name.Equals("Sunflowers"));
			if (index != -1)
			{
				tasks.RemoveAt(index);
			}
			...
			index = tasks.FindIndex(genpass => genpass.Name.Equals("Cactus, Palm Trees, & Coral"));
			if (index != -1)
			{
				tasks.RemoveAt(index);
			}

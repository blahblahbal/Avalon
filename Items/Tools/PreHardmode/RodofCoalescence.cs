...
public class RodofCoalescence : ModItem
{
	...
	{
		Item.DefaultToUseable(false, 60, 60);
	...
	public static bool TeleportPlayer(Player player)
	{
		...
		if ((Main.tile[tileCoords.X, tileCoords.Y].WallType == WallID.LihzahrdBrickUnsafe && !NPC.downedPlantBoss && (Main.remixWorld || tileCoords.Y > Main.worldSurface) ||
			Data.Sets.WallSets.Hellcastle[Main.tile[tileCoords.X, tileCoords.Y].WallType] && !ModContent.GetInstance<SyncAvalonWorldData>().DownedPhantasm) ||
	...
	public static void LimitPointToArea(Player player, int tiles, ref Vector2 pointPoisition)
	{

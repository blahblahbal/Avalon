...
namespace Avalon.Items.Weapons.Magic.PreHardmode
{
    public class LotusLeech : ModItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            {
                NPC npc = Main.npc[i];
                if(npc.active && !npc.friendly && npc.lifeMax > 5 && npc.netID != NPCID.TargetDummy && npc.Distance(player.Center) < 400)
                {
                    Count++;
                    Success = true;
                    ...
                    {
                        var dust = Dust.NewDustPerfect(P.Center, DustID.PurificationPowder, new Vector2(0, 3).RotatedBy(j));
                        dust.noGravity = true;
                    }
                }
                if(Count > 2)

...
public class Zapshock : ModItem
{
	...
	public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
	{
		...
		Projectile.NewProjectile(source,position + position.SafeDirectionTo(AttackPosition).RotatedByRandom(0.3f) * Main.rand.NextFloat(30,40), AttackPosition, type,damage,knockback,player.whoAmI, Utils.RandomNextSeed((ulong)Main.timeForVisualEffects));
		return false;
	}

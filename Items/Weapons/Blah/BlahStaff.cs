...
public class BlahStaff : ModItem
{
	...
	{
		Item.DefaultToStaff(ModContent.ProjectileType<Projectiles.Magic.BlahMeteor>(), 278, 16f, 19, 20f, 15, 15);
	...
	public override bool? UseItem(Player player)
	{
		if (player.whoAmI == Main.myPlayer)
		{
			for (int num9 = 0; num9 < 1; num9++)
			{
				...
				int p = Projectile.NewProjectile(Terraria.Entity.GetSource_None(), vector.X, vector.Y, vector3.X, vector3.Y, ModContent.ProjectileType<Projectiles.Magic.BlahMeteor>(), (int)(player.GetDamage(DamageClass.Magic).ApplyTo(Item.damage)), Item.knockBack, player.whoAmI, 0f, ai2);

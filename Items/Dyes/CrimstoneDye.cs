...
public class CrimstoneDye : ModItem
{
	public override void SetStaticDefaults()
	{
		...
		if (!Main.dedServ)
		{
			GameShaders.Armor.BindShader(
			   Item.type,
			   new ArmorShaderData(Mod.Assets.Request<Effect>("Effects/CrimstoneDye"), "CrimstoneDyeNoise").UseImage("Images/Misc/noise") // Be sure to update the effect path and pass name here.
		   );

			// The following code creates an effect (shader) reference and associates it with this item's type Id.
			GameShaders.Armor.BindShader(
				Item.type,
				new ArmorShaderData(Mod.Assets.Request<Effect>("Effects/CrimstoneDye"), "CrimstoneDye") // Be sure to update the effect path and pass name here.
			);
